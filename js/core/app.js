...
class SpeechToTextApp {
  ...
  initializeWebSocket() {
    if (typeof WebSocketClient === "undefined") {
      console.warn(
        "WebSocketClient not available, intent interpretation disabled",
      );
      return;
    }

    //this.wsClient = new WebSocketClient('ws://localhost:8081'); // nodejs
    this.wsClient = new WebSocketClient("ws://localhost:8082/ws/intent"); // springboot
  ...
  async switchEngine(engineName) {
    ...
    if (!this.currentEngine.isInitialized) {
      ...
      this.updateStatus(
        "ready",
        `${this.currentEngine.getInfo().name} ready to start`,
      );
  ...
  async initializeCurrentEngine() {
    ...
    try {
      ...
      if (this.currentEngineName === "vosk") {
        config.modelUrl =
          "http://localhost:8080/vosk-model-small-en-us-0.15.zip";
      ...
      this.showError(
        "Failed to initialize engine. Please refresh and try again.",
      );
    }
  }
  ...
  async stopRecording() {
    try {
      ...
      if (!this.finalTranscript) {
        ...
        this.elements.interimResults.textContent =
          'Click "Start Recording" to continue...';
        this.elements.interimResults.classList.remove("empty");
      }
